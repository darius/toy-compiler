...
not             => (T.Tnot    (yypos, yypos + 3));
andalso         => (T.Tandalso(yypos, yypos + 7));
orelse          => (T.Torelse (yypos, yypos + 6));

{identifier}	=> (T.Tid (yytext, yypos, yypos + String.size yytext));
[0-9]+		=> (T.Tinteger (parse_int yytext, 
