  ...
  | convert (Ast.App (f, args), c) = 
      trivialize_list (args, 
			 ...
			 case c of 
			   ...
			   | Cjump l => let val v = V (fresh_variable "$r")
...
and convert_if (pred, con, alt, c) = 
      let val l_con = L (fresh_variable "con")
          val l_alt = L (fresh_variable "alt")
       in Loop (l_con, [], convert (con, c),
                Loop (l_alt, [], convert (alt, c), 
